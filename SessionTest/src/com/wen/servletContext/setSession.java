package com.wen.servletContext;
...
public class setSession implements ServletContextListener {
	public void contextInitialized(ServletContextEvent sce) {
		...
		t.schedule(new TimerTask() {
			...
			public void run() {
				...
				/*
				 * for (HttpSession session : list) { long l =
				 * System.currentTimeMillis()-session.getLastAccessedTime();
				 * if(l>5000){//如果时间大于5秒，把session销毁 session.invalidate();//把session销毁
				 * list.remove(session);//从集合中移除 } }
				 */
			}
